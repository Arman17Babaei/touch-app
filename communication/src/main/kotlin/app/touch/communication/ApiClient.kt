...
internal class TouchApiClient : TouchApi {
    ...
    private suspend fun request(s: CommunicationSettings, method: String, path: String, body: JSONObject?, expectBody: Boolean = true): JSONObject = withContext(Dispatchers.IO) {
        ...
        try { val status = c.responseCode; val text = (if (status in 200..299) c.inputStream else c.errorStream)?.bufferedReader(Charsets.UTF_8)?.use { it.readText() }.orEmpty(); if (status !in 200..299) { val (code, message) = ApiJson.error(text); throw ApiException(status, code, message ?: "HTTP $status") }; if (!expectBody || text.isBlank()) JSONObject() else JSONObject(text) } finally { c.disconnect() }
    }
}
