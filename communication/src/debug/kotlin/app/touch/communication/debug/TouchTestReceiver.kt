    ...
    override fun onReceive(context: Context, intent: Intent) = runBlocking {
        ...
        try {
            when (intent.getStringExtra("command")) {
                ...
                "liveAccept" -> communication.acceptLive()
                "liveDecline" -> communication.declineLive()
                "liveBegin" -> Unit
                "liveSample" -> communication.setLiveAmplitude(intent.getIntExtra("amplitude", 0))
                "liveEnd" -> communication.setLiveAmplitude(0)
                "liveMicOn" -> communication.setLiveMicrophoneEnabled(true)
                "liveMicOff" -> communication.setLiveMicrophoneEnabled(false)
                "liveAudioOn" -> communication.setLiveAudioOutputEnabled(true)
                "liveAudioOff" -> communication.setLiveAudioOutputEnabled(false)
